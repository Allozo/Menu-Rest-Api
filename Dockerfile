...
RUN poetry config virtualenvs.create false \
    && poetry install $(if test "$ENVIRONMENT" = production; then echo "--no-dev"; fi) --no-interaction --no-ansi

COPY . .

CMD ["uvicorn", "menu.main:app", "--proxy-headers", "--host", "0.0.0.0", "--port", "8000"]
